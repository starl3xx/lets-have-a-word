 ...
 *
 * Deployed on Base mainnet (UUPS upgrade from JackpotManagerV2)
 *
 * New in V2:
 ...
 *
 * New in V3:
 * - Treasury auto-seed: move creatorProfitAccumulated into jackpot without external ETH
 */
contract JackpotManagerV3 is
    Initializable,
    UUPSUpgradeable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    // ============ State Variables (V2 - DO NOT REORDER) ============
    ...
    /// @notice Emitted when treasury funds are moved into the jackpot (V3)
    event TreasurySeededToJackpot(uint256 amount, uint256 newJackpot, uint256 remainingTreasury);
    ...
    /**
     * @notice Move treasury (creatorProfitAccumulated) funds into the jackpot
     * @dev Only callable by operator. No external ETH needed - pure internal bookkeeping.
     *      No reentrancy guard needed since there are no external calls.
     * @param amount Amount of treasury ETH to move into the jackpot (in wei)
     */
    function seedFromTreasury(uint256 amount) external onlyOperator {
        if (amount == 0) revert InsufficientPayment();
        if (amount > creatorProfitAccumulated) revert InsufficientTreasuryBalance();

        creatorProfitAccumulated -= amount;
        currentJackpot += amount;

        emit TreasurySeededToJackpot(amount, currentJackpot, creatorProfitAccumulated);
    }
