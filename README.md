...
- **The info bar shows the $WORD pool compactly**: The prize pool reads `120M $WORD (≈$20)` instead of `120,000,000 $WORD ($20.00)`. The bar is the tightest surface in the app, and the full-separator form ate its width. The approximation mark is ≈, which the bar already uses for the guess percentage. Only the info bar compacts; every other surface keeps the full form. Two tests cover the new formatter.
