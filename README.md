...
├── data/guess_words_clean.ts  # 4,583 curated words (4,438 through round 35)
...
- **145 words join the list for round 36, and not one round earlier**: ABBOT, AGLOW, QUALM, UVULA and 141 more, taking the list from 4,438 to **4,583**. Round 35 was live when they landed, and that is the whole difficulty: its answer, its 10 bonus words and its 5 burn words were all committed onchain before the round opened, drawn from the smaller list. A word added today **cannot be round 35's answer**, so letting the additions in would have put 145 impossible words on that round's wheel and let a player spend a paid guess on one. Guess validation and the wheel are now round-aware through `isValidGuessForRound(word, roundId)` and `getWordsForRound(roundId)`, with `WORD_LIST_EXPANSION_ROUND = 36`. **The gate is server-side only, on purpose.** The client's typing hint stays on the current list: `pages/index.tsx` holds no live round id, only the archive modal's, and Bugbot caught a first draft that fed that one in, which would have validated a player's typing against whatever historical round they last opened and marked real words invalid in a later round, the answer included. The client check is UX only; `/api/guess` is the authority and refuses the word with no guess consumed. **Answer and bonus selection need no era check at all**, because `createRound` refuses to run while a round is active, so every round created from here is 36 or later. The gate is born with an expiry: once round 36 is underway, `WORDS_THROUGH_ROUND_35`, `WORD_LIST_EXPANSION_ROUND` and their four call sites can be deleted and `WORDS` goes back to being the only list.

- **The expansion would have quietly repriced Superguess**: its legacy tier ladder buckets on `total_dictionary_words - global_guess_count`, and both `/api/superguess/status` and `/api/superguess/purchase` sized that from `getGuessWords().length`. On the round-36 list during round 35 that reports 145 more words remaining than there are, which on the ladder is a **cheaper tier than the round is actually at**. A $WORD round prices from half the live pool so the ladder is only its fallback, but that fallback is exactly what an oracle failure drops into. Both endpoints now size from the round being played, and the pinned quote and the purchase-time recompute read the same basis, so they cannot disagree.

- **The word counts were already wrong before a single word was added**: `CLAUDE.md` claimed 4,437 words with CORE_COMMON at 3,934 and 14 banned; the real numbers were 4,438, 3,847 and 16. The data file's own category comments were stale by the same drift (MORPHOLOGICAL said 402 against a real 505), and `docs/GITBOOK.md` was three expansions behind at 4,374. Every count now states the round-36 number with the through-round-35 number beside it. The in-app FAQ needed no number: it describes the list qualitatively and only cites "12,000+ five-letter entries" for English at large, which is unchanged.

- **The archive was the last public surface still spelling a $WORD prize out in full**: the info bar and the in-game round modal have rounded to three significant digits since 2026-08-15, so the game said "180M $WORD" while `/archive` said "104,888,922 $WORD" for the same kind of number, and `/archive/34` opened with "83,911,137 $WORD jackpot (80%)" over breakdown cards reading 83,911,137 / 5,244,446 / 10,488,892. Every $WORD amount on both public archive pages now rounds the same way the rest of the game does: the per-round rows, the all-time jackpots chip, the prize-pool and jackpot chips, all three breakdown cards, the referrer's earned line and the seed row. **Rounds 1-33 are untouched, by construction rather than by care**: `formatPrizeCompact`'s ETH branch is the same `formatEthAmount(...) + ' ETH'` as `formatPrize`'s, so the flag cannot move an ETH amount, and a test now asserts the two are byte-identical for a jackpot, a seed, a share and a payout entry rather than merely similar. Compaction is an **option on the call**, not a change to the four `formatArchive*` helpers, because `components/admin/ArchiveSection.tsx` shares them and an operator reconciling a row against its payout tx needs every digit.

- **Two chips sitting side by side in the same row rounded by two different rules**: `/archive` had grown its own `formatCompactNumber`, which disagrees with the shared helper at both ends of the scale. It prints "5.7M" where the shared rule prints "5.70M", and it has no billions tier at all, so 1.5B would have rendered as "1500M". The "$WORD bonuses" chip used it while the "$WORD jackpots" chip beside it used full separators, in a row whose own comment says it was rebuilt so the three quantities could be compared. Both now take the shared rule, with the units named at the call site because they arrive differently: the jackpots total is wei, the bonuses total is whole tokens the query already divided down. The jackpots chip was also the **only unguarded `BigInt()` on a public render path** in the app. Everything inside `prize-display.ts` wraps its own; this one would have thrown during render and white-screened the whole archive page, not the one chip, so it is now guarded and degrades to "Unknown".

- **The archive told every bonus-word finder they were paid 5M, including the ones who were not**: `/archive/<n>` captioned the finders list with a flat "5M $WORD each". That was the truth for 128 finds across rounds 1-33 and has been wrong since round 34, when the reward became $1.50 priced by oracle. Verified against production: round 34's eight finds paid **4,435,245 to 5,701,254**, so one flat number was wrong in both directions inside a single round, over by about 11% on most of them and under by about 14% on the first. The caption now states the rule for the era being viewed, and each row shows what that find actually transferred, the same "→ 5.70M $WORD" idiom the in-game modal has used since 2026-08-19. The amount was on the wire the whole time: the page declared `tokenRewardAmount`, a field the API has never sent, while `getBonusWordWinners` returns `rewardWei` read from the claim row. A non-optional field that is `undefined` on every row, and nothing to catch it, since the repo does not typecheck in CI. **The rule text now lives in one function** (`bonusWordRewardRule`), called by both the archive and the modal instead of duplicated as a ternary in each, and a test ties the "$1.50" in the copy to `BONUS_WORD_USD_CENTS` so the constant cannot move without the copy failing. It stays a literal rather than an import because `prize-display.ts` is on the info bar's first-paint path and must not pull `config/economy` into that bundle. The burn-word caption beside it keeps its literal 5M, because `BURN_WORD_AMOUNT` genuinely is fixed in both eras.

- **The live game had the same stale constant, and a finder saw it before the archive ever did**: `guesses.ts` computed the oracle-priced reward, funded the transfer with it, wrote it to the claim row, passed it to the cast... and then returned `tokenRewardAmount: '5000000'` to the player. So the win modal said "+5,000,000 $WORD" and the result banner said "+5M $WORD" for transfers that really ranged 4,435,245 to 5,701,254 in round 34 alone, with the real figure in scope three statements earlier. The field is now `rewardWei`, carrying the exact value that funded the transfer, and the name says its unit: the old one was a whole-token display string sitting beside a wei variable, which is how the two drifted. **The client no longer has a fallback constant.** `pages/index.tsx` defaulted the modal to '5000000' and hardcoded it again for the superguess spectator replay, where the guess log carries no amount at all, so every spectator watched someone "win" 5M. An unknown amount now renders as nothing rather than as the rounds 1-33 number. The modal also stops promising delivery it cannot see: with no txHash the claim is pending or failed, so it says "On its way" instead of "Sent to your wallet". **And it now knows whose find it is showing.** Bugbot caught that the spectator replay passes a null txHash too, so watchers of an already-paid find were being told a payout was on its way to them. A spectator is shown none of the three claims that belong to the finder: the delivery line, the XP and badge line, and the "You found a secret bonus word" heading, which had been telling every watcher they had found it.

- **Lifetime $WORD distributed was a multiplication, not a measurement**: `/api/word-tokenomics` computed it as bonus finds x 5,000,000, so the $WORD sheet every player opens under-reported by roughly 0.7M to 0.9M per find from round 34 on, and the error grew with the price. It now sums `bonus_word_claims` the same way the burn total beside it already summed `word_rewards`. The in-app FAQ and FAQ.md also still described the flat 5M **plus a "2.5M above $150K" tier that is no longer on the award path at all**; both now state the rule, "$1.50 of $WORD, priced the moment you find it".

- **A find that never paid was reporting a payment**: a claim row is written on all three outcomes and carries the amount that WOULD have been sent in each, and `getBonusWordWinners` read it with no status filter. The word is marked claimed before the transfer is attempted, so a `failed` transfer or a `pending` one (the finder had no wallet) still appeared in the list, with a number beside it. Confirmed claims only now. A find with no confirmed claim carries a null amount, which every surface renders as nothing. `retry-bonus-distribution.ts` flips the row to confirmed when it pays, so the number appears once the money actually moves. This was never an archive bug: it reached the in-game round modal the same way, from the same function.

- **The archive pairs every rounded share with the USD it was worth**: rounding to three significant digits drops real tokens, and the archive is the permanent record of what a round paid, so the three breakdown cards now carry the figure that does not move, exactly as the in-game modal does. The value is the round's own `final_jackpot_usd_cents`, priced at its seed-time snapshot when it was archived, which is the whole reason that column exists. ETH rounds get no USD line, because they never had one.

- **A cast and the page it links to now say the same number**: the round-started cast, the round-resolved cast, their push notifications and their cross-posted tweets all rendered the pool through `formatPrize`, so a player read "104,888,922 $WORD" in a cast, tapped it, and read "105M $WORD" on the round it opened. The pool, the payout lines inside the resolved cast, the referral-win cast, the jackpot-milestone cast, the OG share card behind every shared guess and the logged-out front page all compact now. **ETH casts are untouched and cannot change**: they never pass through these branches, `formatEthTrimmed` renders them, and it is deliberately not a prize-display formatter because every existing ETH cast says "0.02 ETH" rather than "0.0200 ETH". Five assertions in `prize-truth-announcements.test.ts` moved to the new strings; what they pin is unchanged, that the figure comes from the row rather than from the contract the resolve just emptied.

- **Deliberately left exact**: a player's own earnings in the stats and referral sheets, the winner's prize line at the moment of winning, and the admin archive table. Those are receipts, not headlines.

### 2026-09-12 (during Round 35)

- **Round 35 auto-started on its own, and a readiness review of the round it started found nine real defects**: round 34 resolved at 11:04:22Z after 26 days, and the `auto-start-round` cron fired round 35 at 17:05:31Z seeded at $39.99 (116,686,114 $WORD)... the first time that cron path had ever run in production, since round 34 was started by hand. It worked and burned no round id. Everything below is what a review of the paths round 35 depends on turned up, fixed on the same day the round opened.

- **Three surfaces announced a prize nobody was playing for**: the round-resolved cast, its push notification and its tweet all rendered the jackpot with `getRoundPrize()`, which reads the live WordJackpot pool... and `announceRoundResolved` runs *after* the onchain `resolveRound` has paid that pool out. So round 34's winner was publicly announced as winning "the 0 $WORD jackpot", with the correct 104,888,922 $WORD sitting one property away on the row that was already being passed in. The same frozen-pool read broke two more surfaces in the other direction: during a $WORD round the contract pool never moves (purchases grow `rounds.prize_pool_word`, and the contract only learns at the single flush before resolve), so the 11:00 UTC daily push reported the seed for the whole round... round 34 grew $20.00 to $26.65, understating by up to 25% by the end... and the jackpot milestone ladder compared a number pinned at 3,999 cents against a 10,000-cent first rung, so **no milestone cast could ever fire on a $WORD round**. PR #318 had raised the rungs correctly; the number being compared to them was the problem. All three now read the row the player's own screen reads, through `prize-display.ts`. The round-*started* cast deliberately keeps its independent contract read, because at start the contract pool is the truth and an independent read is what would catch a seeding mismatch before the bot promises a prize the contract cannot pay. Re-arming the ladder also exposed that its loop had no `break`: the next paid guess would have cast one Neynar post and one tweet for *every* rung the pool had already passed, serially, inside that player's guess request.

- **The reward gate was measuring players against two different $3 bars, 37% apart**: `getPlayBar()` prices the bar from the round's frozen `seed_price_e18` when a round is in scope, and otherwise from `WORD_MARKET_CAP_USD / WORD_TOTAL_SUPPLY_TOKENS`. That env var has never been set, so the fallback resolved to the $25,000 build-time constant... 11,988,000 tokens... while round 35's own frozen bar is ~8,753,000. The path that *accepts* a guess is round-aware; the paths that decide free-guess allocation, the holder bonus, the bar the UI renders and whether the purchase surface is locked are all round-*less*. So a non-grandfathered player holding $3.00 to $4.11 of $WORD cleared round 35's bar on the guess path and was simultaneously allocated zero free guesses, zero holder bonus and shown as locked. For round 34 the two bars were 1.5% apart, which is why the code comment saying they "differ by pennies" went unchallenged. Every round-less caller now resolves the active round's frozen bar, the degraded path reports itself to Sentry instead of silently reverting to the constant, and the same stale constant is out of the holder bonus-guess ladder, which had been handing holders fewer bonus guesses than they qualified for.

- **The onchain guess log had been wedged for 26 days, and nothing would ever have committed a round's tail anyway**: GuessLog holds 19 leaves for round 34 against 3,236 guesses... 5 checkpoints landed in the first 25 minutes and then nothing, including the winning guess that decides 80% of the pool and the whole top-10 ordering. It failed silently by construction: both stop paths return HTTP 200, so Vercel saw a healthy cron for 26 days. Underneath it was a second, independent defect: the checkpoint cron's own docstring promised a final checkpoint for a round that has just resolved, and the code never did it... `postNextCheckpoint` had exactly one caller, targeting the *active* round, and a resolved round has none. Every round would have lost its tail, forever, even with a healthy checkpointer. The cron now sweeps resolved rounds whose log is short, on every tick and in its own error path so a sick live round cannot starve it, and a backlog refusal gets its own quieter alert so round 34 cannot bury a live round-35 stall. Admin → Round Health grew a guess-log card that shows indexed guesses against committed leaves against the contract's own count, plus the exact duplicate and missing indices... which is also the diagnosis for round 34.

- **And the reason it wedged: guess indices were allocated with an unlocked `count(*) + 1`**: two concurrent guesses could read the same count and take the same `guess_index_in_round`, and a duplicate is exactly what makes the checkpoint builder refuse a round as gapped. The comment above it claimed "Uses SELECT FOR UPDATE to prevent race conditions"; it took no lock at all, even when handed a transaction. It is now a `pg_advisory_xact_lock` on the round with a MAX-based allocation. The fix's own first draft introduced a deadlock and the review caught it: the winner transaction took the rounds row `FOR UPDATE` and *then* the advisory lock, while every other guess path takes the advisory lock and then hits the same row implicitly through the foreign key... `FOR KEY SHARE` against `FOR UPDATE`, a textbook ABBA cycle whose victim could be the winning guess. Both locks now live in one named function with the ordering and the reason written above it, and a test drives two real concurrent transactions to prove it: reverse the two lines and it fails with `deadlock detected`.

- **Pulling the kill switch on a $WORD round would have bricked the next one**: it wrote `status='cancelled'` and made no onchain call, so WordJackpot kept holding the round id and every future start would fail its "still active onchain" preflight, every 5 minutes, forever. The DB cancellation still lands first and unconditionally... an emergency control has to work when the chain does not... and the onchain release is attempted after it, failing soft into the recovery path. That recovery is now a button: `recover-stuck-round` grew an arm for the start-side zombie, where the contract holds a round the database does not, and it no longer refuses the most likely real instance of it (a cancelled round with zero guesses). Round creation also stopped burning a permanent round number per failed attempt: the row used to be inserted before the onchain preflight ran, and since the public round number *is* `rounds.id`, every 5-minute retry consumed one. The preflight now runs first and covers all five of WordJackpot's standing revert conditions, `paused()` included, which nothing in the codebase had ever read. `vercel.json` also gained explicit `maxDuration` budgets: the round-start invocation does ~12 seconds of serial onchain work before it even reaches the announcement, and no route in the repo had ever declared one.

- **The archive was about to call round 35's seed 5.24M $WORD**: it derives a round's seed from the previous round's carry, which was the right definition in the ETH era where the carry *was* the opening pool. In the $WORD era the carry merely funds the seed... round 35 opened at 116,686,114 $WORD and its predecessor's carry was 5,244,446, so the public page would have understated the opening pool by 22x. Round 34 dodged it only because its predecessor was an ETH round. Separately, the archive re-derived its own top 10 from the guesses table and re-split the payout pool across *that* list, while the list that was actually paid comes from `getTop10Guessers`, which drops anyone the reward gate refuses and promotes the next eligible FID. With the gate live in production, the archive could name a different person at a rank than the wallet that received the tokens. It now renders the stored `round_payouts` rows: the money column matches the money that moved. A six-wei rounding remainder that the archive was silently dropping confirmed the recomputation.

- **Post-launch cleanup**: the round-34 Wordmark backfill card and its endpoint are deleted, per the rule that one-incident code is born with an expiry. The backfill ran in production on 2026-08-16 and round 34 has resolved; leaving it was not free, because its Trailblazer query carries no round filter, so a re-run would not have been the harmless no-op an operator would reasonably assume.

### 2026-09-08 (during Round 34)

- **The stats sheet tells a first-time visitor their Wordmarks can mint**: a small dismissible callout at the top of the Lexicon card, following the install-prompt seen-key idiom (localStorage, guarded reads because Base App's webview has thrown on storage accessors before). It shows until dismissed rather than flashing exactly once... the point is that the player learns minting exists, not that we said it... and it is gated on the mint feature being live and on the player having at least one earned Wordmark, so nobody reads about a button they cannot see.

- **The mint button connects the wallet itself instead of dead-ending**: in Base App a signed-in player whose webview shed wagmi's stored connection state tapped mint and got "Connect a wallet first"... with no surface anywhere offering to reconnect, because the sign-in screen never shows again once a session exists. The button now connects in place with the sign-in flow's own preference order (injected only when a provider actually exists, since it is configured everywhere but only Base App injects one; Base Account for plain web; never the Farcaster connector, which auto-connects in a host before this button can render) and mints to the freshly connected address, not the stale closure's.

- **The mint sponsorship path is hardened before the paymaster flag ever flips**: the voucher budget finally ran against the real `@upstash/redis` client (via a wire-faithful in-repo REST shim... auto-pipelining, base64 encoding, per-element errors and all), and the review it enabled confirmed two blockers. The GET-then-DECR race let N concurrent requests all pass the budget check and all get sponsored, deterministically under auto-pipelining; the DECR result is now the arbiter, so exactly budget-many spends can ever win. And every re-requested voucher minted a fresh budget key (new deadline, new signature, new key: ~600 bankable vouchers per TTL window), so the budget now keys on the entitlement `(fid, id)` with an NX write that a re-issue cannot refill, and `/api/paymaster` recovers each voucher's signer and refuses anything the attestor never signed. Also: a spend against a key that expired mid-flight no longer leaves an immortal `-1` key (the mirror of the PR #300 INCR bug), refunds restore exactly the keys actually spent, a Redis failure during the budget check fails closed with the graceful RPC error instead of a raw 500, voucher issuance and the paymaster proxy are rate limited, `willSponsor` survives non-string callData, pins the signature length, caps batch width, and its tests finally cover the ERC-8021-suffixed calldata real wallets send. Fifteen integration cases pin all of it, timeouts and expiry races included, with a controllable clock and zero real sleeps.

- **A Farcaster player can actually mint a Wordmark**: the Wordmarks contract went live on Base mainnet today (`0xC526...c232`, owner and attestor verified onchain), and the very first real mint died on "Authentication required". The mint button sent only the wallet-session header... the Base App carrier... and never the Quick Auth token, which is a Farcaster player's only credential the voucher endpoint accepts. PR #300 shipped the button dormant, so the gap had never run. The token now threads StatsSheet → WordmarkDetailModal → WordmarkMintButton and rides the voucher request body, exactly the #295 pattern that fixed the share bonus; both carriers travel and the server picks whichever the player has.

- **Wordmark NFT metadata lives on the domain, by decision**: the pre-deploy plan was to pin the twelve renders to IPFS; the decision now is that `uri()` points at `letshaveaword.fun/api/wordmarks/metadata/` permanently, which makes keeping the domain registered a duty the tokens depend on... and it is recoverable without redeploying, because `setBaseUri` is owner-only. To be a proper permanent metadata target, the route now answers HEAD (indexers preflight with it; it returned 405) and both metadata and image routes send `Access-Control-Allow-Origin: *` for browser-based viewers.

- **The jackpot milestone ladder doubles with the seed**: $100 / $200 / $500 / $1000, replacing $50 / $100 / $250 / $500. The old first rung sat $10 above a fresh $40-seeded round and would have cast "milestone!" before anyone guessed... the exact failure the USD ladder was created to avoid. The first rung stays at 2.5x the seed, the escalated copy stays on the top two rungs, and the per-round dedupe keys are value-based, so the change cannot re-fire anything already announced.
...
- **Eleven modals leave the main chunk, and the first-screen fonts stop hiding**: every flag-gated modal and sheet (share, install, stats, referral, FAQ, winner card, guess packs, and the four $WORD celebration surfaces) was statically imported into the route chunk that gates `sdk.actions.ready()`... code nobody may ever open, paid for on every player's splash screen. All eleven load via `next/dynamic` now, and the `/` route drops another ~35 KB gz (roughly 630 → 276 for the whole day's work). Each was verified conditionally mounted, single-importer, and drop-proof: a slow chunk delays the winner celebration, it can never lose it. `RoundArchiveModal` and `SuperguessPurchaseModal` stay static deliberately (mounted unguarded with `isOpen`, so a bare conversion would fetch them right after hydration anyway), and the install-prompt localStorage helpers move to `src/lib/install-prompt.ts` so the guess flow's one function call stops dragging the whole modal back in. Separately: the wheel hides at opacity 0 until fonts are ready, but no font was preloaded, so cold caches discovered them only after the CSS parsed. The five first-screen weights (300–700) preload from the HTML now, and `/fonts/*` is served immutable... with the rule, written at the config, that a changed font gets a new filename, never an in-place edit.

- **The hot public endpoints stop paying Postgres on cache hits**: every `/api/round-state` poll (15s per client) and every `/api/wheel` mount ran a full `getActiveRound` query even when their Redis caches were hot, because the `activeRoundId` cache key existed but was never read or written. A new `getActiveRoundId()` wires it... and the constraint that shaped it is written into the code: `getActiveRound()` returns the *decrypted answer*, so the cache stores the integer id (or a no-round sentinel) and nothing else, ever, with a test that scans the whole mocked store for the answer string. The adversarial pass caught something worse than slowness before it shipped: under a cached id, the endpoints' fetch functions could reach `ensureActiveRound()`, which *creates* rounds... a poll landing in the seconds between a win locking the round and the transition invalidation would have minted a fresh answer, committed it onchain, and announced it, from an anonymous request. The fetch functions now take the id they already hold and can never reach a creation path. From the same pass: the global "has anyone Superguessed" boolean gets a 10s cache as a *separate opt-in function* so the purchase endpoint keeps its uncached query (money points read uncached, always); `/api/wheel/wrong-guesses` stops running its own drifted active-round select (the drift class that once disclosed an answer) and gains the same rate limiter and CDN header as `/api/wheel`; the dead-day wheel branch gets a short `s-maxage`; round start and kill-switch cancel invalidate the id cache; and every cache read coerces string round-trips, because a `typeof` guard on this exact Redis froze the price bar for two days once.

- **The wheel paints instantly, and the ticker stops waiting for the handshake**: the wheel showed "Loading..." for a full API round trip while `/api/wheel` sent back the same 4,400-odd words already sitting in the client bundle. It now initializes from the bundled list with every word unguessed... byte-for-byte the shape the API itself serves between rounds, so nothing about the answer leaks that was not already public... and the fetch replaces it with authoritative statuses when it lands. The failure mode flips from bad to fine: the old empty-array state left an *invisible* wheel forever if the fetch failed (the centering gate never fired at zero words); now the full wheel stands and the wrong-guess poll fills statuses in. Separately, `TopTicker` mounts below the boot-screen early return, so its first `/api/round-state` fetch waited out the entire mini app handshake (up to ~2s on a slow host) for public data needing no auth. A module-scope prefetch now starts the request at JS evaluation, before hydration; the ticker's first paint consumes it, single-use and age-guarded so a player who lingers on the sign-in screen gets a fresh fetch instead of a minutes-old prize figure.

- **Sentry comes off the critical path, and Solana leaves the bundle entirely**: the splash screen dismisses when the route JS finishes, and ~155 KB gz of that JS was the Sentry SDK... loaded before hydration by Next convention, for telemetry that matters most *after* the app is running. `instrumentation-client.ts` is now a scheduler: two tiny window handlers buffer any error thrown before the SDK arrives, the full SDK loads after first paint on idle, and the buffer replays into it. `ErrorBoundary` force-loads at catch time, because a crash is exactly the moment the download is worth it. The honest cost: an error in a session that dies before ever reaching idle goes unreported. Separately, `@solana/web3.js` (~60 KB gz) rode in through `@farcaster/miniapp-core`'s CJS star re-export, un-tree-shakeable, in a Base-only game with zero Solana code... a `turbopack.resolveAlias` now points it at an in-repo stub whose members throw a clear message if a Solana path is ever really driven. Measured: the `/` route drops from ~448 to ~311 KB gz, which with the morning's work makes ~630 → ~311 for the day. The stub must be re-checked on every `@farcaster/miniapp-sdk` or `@base-org/*` upgrade, and the comment on it says so.
...
- **Bonus Words** (`src/lib/guesses.ts`, `src/lib/word-lists.ts`)
  ...
  - Finding one awards $1.50 of $WORD directly to the player's wallet, oracle-priced at the moment of the find (a flat 5M for rounds 1-33)
