...
- **Stats shares to X outside a Farcaster host**, with the icon following the destination rather than showing a Farcaster arch on a button that opens X. The project's X handle is deliberately omitted until `NEXT_PUBLIC_X_HANDLE` is set: "@letshaveaword" is the Farcaster account, and guessing the X name would @-mention whoever holds it from every player's post.
