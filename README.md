...
- **The test suite now refuses to run against a non-test database**: it is destructive by design — `retireActiveRounds()` resolves whatever round it finds before *every* test, and several files create and resolve rounds freely — yet it read whatever `DATABASE_URL` was in the environment with no idea which database that was. Pointed at production it created **90 rounds**, and because starting a round calls `announceRoundStarted()`, the announcer posted **87 casts from the live bot account** before it was stopped. Nothing in the suite objected, because nothing was watching. `setup.ts` now refuses any database that is not on localhost or named `*test*` (CI's is `lhaw_test`, so CI is unaffected), with an explicit opt-out for anyone who really means it. A second layer forces `ANNOUNCER_ENABLED=false`, `TWITTER_ENABLED=false` and drops the Neynar signer at setup top level — before any test file can import `announcer.ts` and read the flag into a module-level const — so a run cannot reach the outside world even if the database check is ever loosened. Verified by re-running the exact command that caused the incident and watching it refuse.
