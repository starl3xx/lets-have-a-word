...
- **External links now work outside the mini app**: every "View on BaseScan" button, "tx ↗" link and player-profile tap on the public archive and verify pages called `sdk.actions.openUrl()` / `viewProfile()` directly — an RPC to the Farcaster host, which on the plain web has nobody listening, so the promise never resolved and the click silently did nothing. The pages themselves work fine in a browser (they fetch public APIs), which is exactly what made the dead controls reachable. All 13 `openUrl` call sites now go through one `ExternalLink` component that renders a real `<a target="_blank">` — web visitors get an actual link, cmd-click and right-click-copy included — and intercepts the click to route through the SDK only when running inside a mini-app host, where the bare anchor is the unreliable one. The split is decided by a new `useIsInMiniApp` hook rather than per call site, and it defaults to web behaviour, so a slow host handshake degrades to a working link instead of a dead button. Profile taps fall back to the player's `farcaster.xyz` profile page. The archive page's "Verify round" button also stops opening the app's own `/verify` page through the SDK with a hardcoded `www.letshaveaword.fun` origin; it is an in-app `next/link` navigation now.
