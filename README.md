...
- **Shares go where the player actually is**: Stats, referrals and Wordmarks now open the X composer outside a Farcaster host, handing the compose to the installed X app via `twitter://post` rather than a logged-out X inside an in-app browser tab (the https intent stays as the fallback, guarded on page visibility so returning from the app cannot open a second composer), styled black rather than Farcaster purple, and the winner card hides its Farcaster button there rather than offering a share whose only possible outcome is failure. Icons follow the destination. The two handles are now separate constants and never interchangeable... the X account is **@letshaveaword_** and the Farcaster account is **@letshaveaword**, so posting the Farcaster handle to X (which the winner card's X share did) tags whoever holds that name there, from a jackpot announcement.
